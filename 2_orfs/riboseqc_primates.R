##R CMD INSTALL -l /home/jruizor/lib/ RiboseQC.tar.gz
##R CMD INSTALL -l /home/jruizor/lib/ ORFquant.tar.gz

.libPaths("/home/jruizor/lib/")

#library("devtools")
#library("stringr")
library("base")
library("RiboseQC", lib.loc="/home/jruizor/lib/")
library("ORFquant", lib.loc="/home/jruizor/lib/")


RiboseQC_analysis <- function(annotation_file, bam_files, genome_seq, read_subset = TRUE, readlength_choice_method = "max_coverage", 
                              stranded = TRUE, normalize_cov = TRUE, chunk_size = 5000000L, 
...
species = args[2]
cores = args[3]
opt = args[4] #positive: standard, negative: custom. 0: index. 1: riboseQC. 2: ORFquant
if (opt == "1" | opt == "2"){
  dir = "../1_mapping/fasta/input/"
} else if (opt == "-1" | opt == "-2"){
  dir = "../1_mapping/custom/"
}


if (species == "hsa"){
  gtf = args[5] #"../1_mapping/annotation/Homo_sapiens.GRCh38.98.stringtie2021.fixed.gtf"
  annot_name = args[6] #"hsa38.98.stringtie2021"
  sp = "Homo.sapiens"
  annot = args[7] #../1_mapping/fasta/input/Homo_sapiens.GRCh38.98.stringtie2021.fixed.gtf_Rannot"
  bit = "../1_mapping/fasta/Homo_sapiens.GRCh38.dna.toplevel.2bit"
  fa = "../1_mapping/fasta/Homo_sapiens.GRCh38.dna.toplevel.fa"
} else if (species == "ptr"){
  gtf = args[5] #"../1_mapping/annotation/Pan_troglodytes.Pan_tro_3.0.98.stringtie2021.fixed.gtf"
  annot_name = args[6] #"Pantro3.0.98.stringtie2021"
  sp = "Pan.troglodytes"
  annot = args[7] #../1_mapping/fasta/input/Pan_troglodytes.Pan_tro_3.0.98.stringtie2021.fixed.gtf_Rannot
  bit = "../1_mapping/fasta/Pan_troglodytes.Pan_tro_3.0.dna.toplevel.2bit"
  fa = "../1_mapping/fasta/Pan_troglodytes.Pan_tro_3.0.dna.toplevel.fa"
} else if (species == "mml"){
  gtf = args[5] #"../1_mapping/annotation/Macaca_mulatta.Mmul_10.98.stringtie2021.fixed.gtf"
  annot_name = args[6] #"Mmul10.98.stringtie2021"  
  sp = "Macaca.mulatta"
  annot = args[7] #../1_mapping/fasta/input/Macaca_mulatta.Mmul_10.98.stringtie2021.fixed.gtf_Rannot
  bit = "../1_mapping/fasta/Macaca_mulatta.Mmul_10.dna.toplevel.2bit"
  fa = "../1_mapping/fasta/Macaca_mulatta.Mmul_10.dna.toplevel.fa"
} else if (species == "ggo"){
  gtf = args[5] #"../1_mapping/annotation/Gorilla_gorilla.gorGor4.98.stringtie2021.fixed.gtf"
  annot_name = args[6] #"gorGor4.98.stringtie2021"
  sp = "Gorilla.gorilla"
  annot = args[7] #../1_mapping/fasta/input/Gorilla_gorilla.gorGor4.98.stringtie2021.fixed.gtf_Rannot"
  bit = "../1_mapping/fasta/Gorilla_gorilla.gorGor4.dna.toplevel.2bit"
  fa = "../1_mapping/fasta/Gorilla_gorilla.gorGor4.dna.toplevel.fa"
} else if (species == "mmu"){
  gtf = args[5] #"../1_mapping/annotation/Mus_musculus.GRCm38.98.stringtie2021.fixed.gtf"
  annot_name = args[6] #"mmu.98.stringtie2021"
  sp = "Mus.musculus"
  annot = args[7] #../1_mapping/fasta/input/Mus_musculus.GRCm38.98.gtf_Rannot
  bit = "../1_mapping/fasta/Mus_musculus.GRCm38.dna.toplevel.2bit"
  fa = "../1_mapping/fasta/Mus_musculus.GRCm38.dna.toplevel.fa"
} else if (species == "het"){
  gtf = args[5] #"../1_mapping/annotation/Heterocephalus_glaber_male.HetGla_1.0.98.stringtie2021.fixed.gtf"
  annot_name = args[6] #"het.98.stringtie2021"
  sp = "Heterocephalus.glaber"
  annot = args[7] #../1_mapping/fasta/input/X.98.gtf_Rannot
  bit = "../1_mapping/fasta/Heterocephalus_glaber_male.HetGla_1.0.dna.toplevel.2bit"
  fa = "../1_mapping/fasta/Heterocephalus_glaber_male.HetGla_1.0.dna.toplevel.fa"
} else if (species == "rat"){
  gtf = args[5] #"../1_mapping/annotation/Rattus_norvegicus.Rnor_6.0.98.stringtie2021.fixed.gtf"
  annot_name = args[6] #"rat.98.stringtie2021"
  sp = "Rattus.norvegicus"
  annot = args[7] #../1_mapping/fasta/input/X.98.gtf_Rannot
  bit = "../1_mapping/fasta/Rattus_norvegicus.Rnor_6.0.dna.toplevel.2bit"
  fa = "../1_mapping/fasta/Rattus_norvegicus.Rnor_6.0.dna.toplevel.fa"
} else if (species == "opp"){
  gtf = "../1_mapping/annotation/Monodelphis_domestica.ASM229v1.98.stringtie2020.fixed.gtf"
  annot = "../1_mapping/fasta/input/Monodelphis_domestica.ASM229v1.98.stringtie2020.fixed.gtf_Rannot"
  bit = "../1_mapping/fasta/Monodelphis_domestica.ASM229v1.dna.toplevel.2bit"
  fa = "../1_mapping/fasta/Monodelphis_domestica.ASM229v1.dna.toplevel.fa"
  sp = "Monodelphis.domestica"
  annot_name = "opp38.98.stringtie2020"
} else if (species == "gal"){
  gtf = args[5] #"../1_mapping/annotation/Gallus_gallus.GRCg6a.98.stringtie2020.fixed.gtf"
  annot_name = args[6] #"GRCg6a.98.stringtie2020"
  sp = "Gallus.gallus"
  annot = args[7] #../1_mapping/fasta/input/Gallus_gallus.GRCg6a.98.gtf_Rannot
  bit = "../1_mapping/fasta/Gallus_gallus.GRCg6a.dna.toplevel.2bit"
  fa = "../1_mapping/fasta/Gallus_gallus.GRCg6a.dna.toplevel.fa"
} else if (species == "hsa98"){
  gtf = "../1_mapping/annotation/Homo_sapiens.GRCh38.98.gtf"
  annot = "../1_mapping/fasta/input/Homo_sapiens.GRCh38.98.gtf_Rannot"
  bit = "../1_mapping/fasta//Homo_sapiens.GRCh38.dna.toplevel.2bit"
  fa = "../1_mapping/fasta/Homo_sapiens.GRCh38.dna.toplevel.fa"
  sp = "Homo.sapiens"
  annot_name = "hsa38.98"
} else if (species == "rat98"){
  gtf = "../1_mapping/annotation/Rattus_norvegicus.Rnor_6.0.98.gtf"
  annot = "../1_mapping/fasta/input/Rattus_norvegicus.Rnor_6.0.98.gtf_Rannot"
  bit = "../1_mapping/fasta/Rattus_norvegicus.Rnor_6.0.dna.toplevel.2bit" 
  fa = "../1_mapping/fasta/Rattus_norvegicus.Rnor_6.0.dna.toplevel.fa"
  sp = "Rattus.norvegicus"
  annot_name = "Rnor_6.0.98"
} else if (species == "mmu98"){
  gtf = "../1_mapping/annotation/Mus_musculus.GRCm38.98.gtf"
  annot = "../1_mapping/fasta/input/Mus_musculus.GRCm38.98.gtf_Rannot"
  bit = "../1_mapping/fasta/Mus_musculus.GRCm38.dna.toplevel.2bit"
  fa = "../1_mapping/fasta/Mus_musculus.GRCm38.dna.toplevel.fa"
  sp = "Mus.musculus"
  annot_name = "mmu38.98"
...
####2bit and gtf should have the same chrms
####Use Ensembl GTFs
if (opt == "0"){
  prepare_annotation_files(annotation_directory = "../1_mapping/fasta/input", twobit_file = bit,
                           gtf_file = gtf,scientific_name = sp,
                           annotation_name = annot_name,export_bed_tables_TxDb = F,forge_BSgenome = T,create_TxDb = T)
  quit(save="no")
}

if (opt == "-0"){
  prepare_annotation_files(annotation_directory = "../1_mapping/custom/", twobit_file = bit,
                           gtf_file = gtf,scientific_name = sp,
                           annotation_name = annot_name,export_bed_tables_TxDb = F,forge_BSgenome = T,create_TxDb = T)
  quit(save="no")
}
...
#Recursive analysis
riboseqc_analysis <- function(bam,gtf,fa){
  print(bam)
  tryCatch(
    expr = {
      RiboseQC_analysis(annotation_file = annot, bam_file = bam, genome_seq = fa,
                        read_subset = F, readlength_choice_method = "max_coverage", chunk_size = 5000000L, write_tmp_files = T, dest_names = NA, 
                        rescue_all_rls = FALSE, fast_mode = F, create_report = T, sample_names = NA, report_file = NA, extended_report = F, pdf_plots = T)
...
orfquant <- function(bam,gtf,cores,pref){
  run_ORFquant(for_ORFquant_file = paste0(bam,"_for_ORFquant"), annotation_file = annot, n_cores = cores, unique_reads_only = TRUE, stn.orf_find.cutoff_fr_ave = 0.7, prefix = paste0(bam,pref))
  plot_ORFquant_results(for_ORFquant_file = paste0(bam,"_for_ORFquant"),ORFquant_output_file = paste0(bam,"_ORFquant_final_ORFquant_results"), annotation_file = annot)
  create_ORFquant_html_report(input_files = paste0(bam,"_ORFquant_final_ORFquant_results_plots/",basename(bam),"_ORFquant_ORFquant_plots_RData"),input_sample_names = basename(bam), output_file= paste0(bam,"_ORFquant.html"))
}


#files  <- list.files(dir, recursive = T, full.names = T,pattern = ".bam$")
  #lapply(files,riboseqc_analysis)

if (opt == "1" | opt == "-1"){
#  install.packages(paste0("../1_mapping/fasta/input/BSgenome.",sp,".",annot_name),repos=NULL, type="source")
  library(paste0("BSgenome.",sp,".",annot_name), lib.loc = "/fast/home/j/jruizor/lib/",character.only = TRUE)
  riboseqc_analysis(bam,gtf,fa)
}

if (opt == "2" | opt == "-2"){
#  install.packages(paste0("../1_mapping/fasta/input/BSgenome.",sp,".",annot_name),repos=NULL, type="source")
  library(paste0("BSgenome.",sp,".",annot_name), lib.loc = "/fast/home/j/jruizor/lib/",character.only = TRUE)
  orfquant(bam,gtf,cores,"_ORFquant")
}
